...
async def set_afk(afk_e):
    ...
    if string:
        AFKREASON = string
        await afk_e.edit(f"🔥PESAN DARI HAPIS\n╭╼═══╾╼═══╾╼═══╾╼═══╾ \n┣ {ALIVE_NAME} ᴀꜰᴋ ᴅᴜʟᴜ\n┣ ᴀʟᴀꜱᴀɴ : {AFKREASON}\n╰╼═══╾╼═══╾╼═══╾")
    else:
...
async def type_afk_is_not_true(notafk):
    ...
    user = await bot.get_me()  # pylint:disable=E0602
    last = user.last_name
    if last and last.endswith("🔥 HAPIS LAGI NGEBUCIN"):
        last1 = last[:-12]
    else:
        last1 = ""
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.respond("🔥HAPIS KEMBALI DENGAN MEMBAWA SEJUTA KERINDUAN")
        time.sleep(3)
        await msg.delete()
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"🔥 HAPIS LAGI NGEBUCIN\n╭╼═══╾╼═══╾╼═══╾╼═══╾ \n┣ {ALIVE_NAME} TUAN HAPIS\n┣ ꜱᴇᴊᴀᴋ: {afk_since}\n┣ ᴀʟᴀꜱᴀɴ: {AFKREASON}\n╰╼═══╾╼═══╾╼═══╾")
                else:
                ...
                if USERS[mention.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await mention.reply(f"🔥PESAN DARI HAPIS\n╭╼═══╾╼═══╾╼═══╾ \n┣ {ALIVE_NAME} TUAN HAPIS\n┣ TUAN HAPIS : {afk_since}\n┣ TUAN HAPIS: {AFKREASON}\n╰╼═══╾╼═══╾")
                    else:
                        await mention.reply(str(choice(AFKSTR)))
